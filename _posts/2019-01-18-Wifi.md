...
This last attack for WPA2 is nice because it's pretty stealthy. No clients need to be disconnected to the access point for it to work. Apparently, Wifite2 comes with pmkid tools built in. This used to be a pain in the ass but now all you have to do is run the previous command with these tools installed. If it's gonna be automated this much I don't see why the pmkid tools aren't packaged too. The dump tool gets the data, hcx converts it to a format [hashcat](https://hashcat.net/forum/thread-7717.html) can read.
...
```
I noticed probesniffer hasn't been updated in a long time, so if it isn't working a simple dump is better than nothing. Pipe this command to a file to save it for later. For this you'll need [tcpdump](https://www.tcpdump.org/manpages/tcpdump.1.html).
```
